...
function createHttpFacilitator() {
  const url = process.env.FOREMAN_FACILITATOR_URL;
  if (!url) return null;
  const token = process.env.FOREMAN_FACILITATOR_TOKEN;
...
function createLocalFacilitator() {
  const privateKey = process.env.FOREMAN_FACILITATOR_PRIVATE_KEY;
  if (!privateKey) return null;
  if (!/^0x[a-fA-F0-9]{64}$/.test(privateKey)) {
    throw new PaymentConfigurationError("FOREMAN_FACILITATOR_PRIVATE_KEY must be a 32-byte hex key");
  }
  ...
  registerExactEvmScheme(facilitator, { signer, networks: XLAYER.network, simulateInSettle: true });
  return facilitator;
}
...
export function createPaymentService({ facilitator, chain } = {}) {
  ...
  function getFacilitator() {
    ...
    if (!resolvedFacilitator) throw new PaymentConfigurationError("Foreman payment facilitator is not configured");
    return resolvedFacilitator;
  }

  async function verify(raw, requirements) {
    ...
    return { payload, payer: getAddress(result.payer) };
  }
  ...
  async function settle(verified, requirements) {
    ...
    if (!result?.success || !/^0x[a-fA-F0-9]{64}$/.test(result.transaction || "")) {
    ...
    const amount = result.amount || requirements.amount;
    if (String(amount) !== String(requirements.amount)) throw new PaymentVerificationError("settlement_amount_mismatch");
